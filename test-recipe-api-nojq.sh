...
if echo "$ACCORD_RESPONSE" | grep -q '"_id"'; then
    ACCORD_ID=$(echo "$ACCORD_RESPONSE" | python3 -c "import sys, json; print(json.load(sys.stdin).get '._id')
...
if echo "$RECIPE_CREATE" | grep -q '"_id"'; then
    RECIPE_ID=$(echo "$RECIPE_CREATE" | python3 -c "import sys, json; print(json.load(sys.stdin).get '._id')
...
if echo "$VERSION_CREATE" | grep -q '"_id"'; then
    VERSION_ID=$(echo "$VERSION_CREATE" | python3 -c "import sys, json; print(json.load(sys.stdin).get '._id')
...
if echo "$INGREDIENT_ADD" | grep -q '"_id"'; then
    INGREDIENT_ID=$(echo "$INGREDIENT_ADD" | python3 -c "import sys, json; print(json.load(sys.stdin).get '._id')
...
if echo "$NOTE_CREATE" | grep -q '"_id"'; then
    NOTE_ID=$(echo "$NOTE_CREATE" | python3 -c "import sys, json; print(json.load(sys.stdin).get '._id')
...
if echo "$COLLECTION_CREATE" | grep -q '"_id"'; then
    COLLECTION_ID=$(echo "$COLLECTION_CREATE" | python3 -c "import sys, json; print(json.load(sys.stdin).get '._id')
